...
import { X, Mail, Lock, User, Loader2, ShieldCheck, AlertCircle, Eye, EyeOff, Phone, LoaderPinwheel } from "lucide-react";
    ...
    const handleSubmit = async (e) => {
        ...
        try {
            ...
            if (res.data.success) {
                ...
                onRefresh();
                onClose();
                setFormData({ first_name: "", last_name: "", email: "", mobile: "", role: "", description: "", password: "" });
                setTouched({ first_name: false, last_name: false, mobile: false, email: false, role: false, description: false, password: false });
            }
        } catch (err) {
            ...
            <div className="bg-white w-full max-w-lg rounded-[40px] shadow-2xl overflow-hidden">
                <div className="bg-slate-900 p-8 text-white flex justify-between items-center">
                    <div>
                        ...
                        <p className="text-slate-400 text-xs font-bold uppercase tracking-widest mt-1">Validation Active</p>
                    </div>
                ...
                <form onSubmit={handleSubmit} className="p-8 space-y-4" noValidate>
                    ...
                    <div className="grid grid-cols-2 gap-4">
                        <div className="space-y-1">
                            ...
                            <div className="relative">
                                <LoaderPinwheel className={`absolute left-4 top-1/2 -translate-y-1/2 ${getFieldError('role') ? 'text-rose-500' : 'text-slate-300'}`} size={18} />
                                <input type="text" name="role" value={formData.role} onChange={handleChange} onBlur={handleBlur} placeholder="Admin/Teacher"
                                    className={`w-full bg-slate-50 border rounded-2xl py-4 pl-12 pr-4 text-sm font-bold outline-none transition-all ${getFieldError('role') ? 'border-rose-500 ring-4 ring-rose-500/10' : 'border-slate-100 focus:ring-4 focus:ring-indigo-500/10'}`} />
                            </div>
                        </div>
                    ...
                    <div className="space-y-1">
                        ...
                        <div className="relative">
                            <LoaderPinwheel className={`absolute left-4 top-1/2 -translate-y-1/2 ${getFieldError('description') ? 'text-rose-500' : 'text-slate-300'}`} size={18} />
                            <input type="text" name="description" value={formData.description} onChange={handleChange} onBlur={handleBlur} placeholder="Min 10 characters description"
                                className={`w-full bg-slate-50 border rounded-2xl py-4 pl-12 pr-4 text-sm font-bold outline-none transition-all ${getFieldError('description') ? 'border-rose-500 ring-4 ring-rose-500/10' : 'border-slate-100 focus:ring-4 focus:ring-indigo-500/10'}`} />
                        </div>
                    </div>

                    <div className="space-y-1">
                        <label className="text-[10px] font-black text-slate-400 uppercase tracking-widest ml-1">Email (@gmail.com)</label>
                        <div className="relative">
                    ...
                    <div className="pt-2">
                        <button disabled={loading || !isFormValid} type="submit"
                            className={`w-full py-5 rounded-[22px] font-black transition-all ${isFormValid ? 'bg-indigo-600 text-white shadow-xl shadow-indigo-100 hover:bg-indigo-700' : 'bg-slate-100 text-slate-400 cursor-not-allowed'}`}>
